    ...
    return (
        <div className="flex items-center gap-2 md:gap-4 text-text-primary">
            <div className="flex items-center gap-1" title={`Ранг: ${rank.name}`}>
                <span className="text-xl md:text-2xl" style={{ color: '#52D186' }}>{rank.icon}</span>
                <span className="font-bold text-base md:text-lg">{level}</span>
            </div>

            <div className="flex items-center gap-1">
                <span role="img" aria-label="Hearts">🤍</span>
                <span className="text-xs md:text-sm font-semibold">{focusCrystals}</span>
            </div>
